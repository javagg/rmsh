// Mesh surface shader with Blinn-Phong lighting and per-face color
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // Ambient
    let ambient_color = in.base_color * 0.18;

    // Diffuse
    let diff = max(dot(normal, light_dir), 0.0);
    let diffuse_color = in.base_color * diff;

    // Specular (Blinn-Phong)
    let half_dir = normalize(light_dir + view_dir);
    let spec = pow(max(dot(normal, half_dir), 0.0), 32.0);
    let specular_color = vec3<f32>(1.0, 1.0, 1.0) * spec * 0.25;
